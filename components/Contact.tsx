...
export default function Contact() {
      ...
      if (!response.ok) {
        if (response.status === 429) {
          const retryAfter = Number(
            response.headers.get("Retry-After") || result.retryAfterSeconds,
          );
          setStatus("error");
          setStatusMessage(
            Number.isFinite(retryAfter) && retryAfter > 0
              ? `Terlalu banyak percobaan. Coba lagi sekitar ${Math.ceil(retryAfter / 60)} menit lagi.`
  ...
  return (
    <section id="kontak" className="contact-section scroll-mt-24">
      <div className="site-container section-space">
        ...
        <h2 className="font-display text-[clamp(1.75rem,3.6vw,2.625rem)] font-extrabold tracking-tight">
          Ide besar dimulai dari percakapan kecil.
        </h2>
        ...
        <div className="mt-7 grid gap-8 md:grid-cols-[1fr_1.2fr] md:gap-12">
          <div>
            <p className="max-w-[400px] text-[16.5px] leading-relaxed text-muted">
              Website pertama atau sistem untuk merapikan pekerjaan? Ceritakan
              ide dan kebutuhan Anda. Kita cari langkah awalnya bersama.
            </p>
